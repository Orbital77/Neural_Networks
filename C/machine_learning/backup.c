...
void free_memory_network(Network *network, int n_layers)
{
	for (int i = 0; i < (n_layers - 1); i++)
	{
		free(network->layer[i]);
		network->layer[i] = NULL;
	}
	free(*network->layer);
	*network->layer = NULL;
	free(network->visual);
...
//		}
//	}
	free_memory_network(network, n_layers);
}
...
int main(int argc, char **argv)
{
	if (argc == 3)
	{
		...
		create_network(argv[1], argv[2], g_layers, &layer_0, &network, 2);

		free_memory_network(&network, 2);
