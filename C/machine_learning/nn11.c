...
//This program utilizes the N_Layer and V_Layer structures to create the 'Network' structure.
//At this time, all values are used in calculations and/or functions expcept for the variable 'p_neurons'.
//Values can be inserted into these structures by loading a formatted file. 'output.dat' and 'input.dat' are both used as arguments for this purpose.
...
//Gets a random number in between the min and the max values given to the function.
...
//Transpose function for weights
...
//The dot product for:
//	a vector and anoter vector
//	a vector and a matrix
//	a matrix and a another matrix
...
//Functions to display different data types.
...
//Functions for allocating and freeing memory for the structs
...
//Activation functions for a layer of neurons
...
int main(int argc, char **argv)
{
	if (argc == 4)
	{
//		int network_shape[] = {4, 8, 12, 12, 8, 4};
//		Network *network = create_network(argv[2], 6, network_shape);

//		activate_network(network);

//		display_network(network);

//		save_network(argv[3], network);

//		free_memory_network(network, 6);
...
//This program works by either creating a randomly initialized neural network, or by loading a pre-existing network from a set of files.
//To load a file, you must provide a file similar to input.dat, and a file similar to output.dat
//In this version of the program, the network is missing a way to optimize the weights and biases, but has activation functions that nn10.c does not.
//With this configuration, you can try running the network by giving nn11.c the following syntax: ./nn11.c output.dat input.dat output.dat
